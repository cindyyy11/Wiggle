...
Checked: 2 web unit files / 9 tests pass (`mathSceneryLayout` 5 and `Numeria` 4), with TypeScript and universe ESLint also passing. Visual: the globe was inspected at 1440 x 900 and 390 x 844; rotated far-side checks found no wide bare band, no filler prop sitting on a plateau or inside a cluster's core, and the four lands still read as visually distinct. There were no browser console warnings or errors and no visible quick-pan regression; the counts were left as tuned, and the clearing radius was later raised from .45 to .65 in review so the filler keeps clear of the plateaus and cluster centres by a wider margin.
