...
- The floating and walking astronauts were enlarged and made tappable (hover waves, tap hops or somersaults). The walking explorer now uses the same polished rig as the floating one (glossy visor with a smiling face and blinking eyes, gold trim, backpack, a real walking stride) and, once it has stopped for 0.4 s, turns to face the camera; that turn is drawn only, so the chase camera does not follow it round. The chase camera also eases into a close, near-level portrait view (`PORTRAIT_*` in `cameraMotion.ts`, driven by `ExplorerInput.portrait`) once it has stopped, so the face is seen straight on; it returns to the high chase view as soon as the explorer walks again, and reduced motion keeps the existing camera. Hover, tap, walking and stopping were exercised in Chrome with no page errors; this is not a physical-device check.
...
The Numeria (math) planet now has a whole-globe filler layer (`MathLivingScenery`) of small, sparse versions of its own props — trees, hills, berries and number-boxes, chosen by each filler point's nearest land — plus a light instanced dust of ground-detail dots, so the terrain between Fraction Forest, Number Valley, Geometry Ridge and Crystal Crater no longer reads as bare. The four existing per-land clusters, the region plateaus and `ShapeSparkles` are unchanged; the filler layer's points are scattered with the same Fibonacci-sphere method Science's `LivingScenery` uses, and dropped within `CLEARING_RADIUS` (.65) of a land's center. That clears the region plateaus with margin and the dense middle of each cluster, but not every cluster's full extent — Fraction Forest's outermost trees reach ~.677 from its centre, so a filler prop can still sit among them.

Checked: 2 web unit files / 10 tests pass (`mathSceneryLayout` 5 and `Numeria` 5), with TypeScript and universe ESLint also passing. Visual: after the clearing radius was raised from .45 to .65, the globe was re-inspected in the dev server at `/?world=math` via the high-quality path (`resolveQuality(preference, …)` after reverting a local `"low"` override); the tall side-browser viewport (~300 px wide) and zoomed-out / whole-planet views on Geometry Ridge were used. Filler props are visible in the gaps between lands; no hero prop was seen on a region plateau or inside a cluster's dense core; the four lands still read as visually distinct. Residual sparse rings immediately around each land are expected from the .65 clearing, which clears plateaus and cluster centres but not every outer Forest tree. An earlier pass at .45 used 1440 × 900 and 390 × 844. No browser console errors were observed during the pass; the counts were left as tuned.
...
Checked: 90 web unit files / 525 tests pass, including the extracted bench canvas and bench space against the unchanged Round 1 tests, the pure dwell selector and puzzle reducer, a per-region set-consistency test against the lesson data, and a full puzzle-by-puzzle play for every region with simulated hand selections including wrong answers, hand loss and a re-open restart. Browser: the numeria-hand-sessions spec (camera requested on entry for all three regions, adult help and Try again, no radio buttons, Escape, no horizontal scroll, Fraction Forest unaffected) passes at desktop and 390 px, and universe, hover-contrast, subject-worlds and mission still pass. Only Number Valley's workbench was inspected in screenshots at 1440 x 900 and 390 x 844; Geometry Ridge and Crystal Crater's 3D layer has not been visually checked by a human — the automated tests mock out `AnswerBenchScene` everywhere, and the browser spec only exercises the camera-denied "ask an adult" screen, so those two workbenches have only the structural/data checks of the `answerSets.test.ts` consistency suite, not a rendered look.

Not verifiable automatically: real hand tracking (the 1200 ms hold, the 300 ms leave grace, tracking-loss reset). All three rounds of the camera-first effort are now complete for the lands and regions in scope; moving Magnet Lands onto the shared frame remains optional future work. A real-camera check by the owner should also cover: (a) confirming Geometry Ridge's mountain shape visually matches its corner beacons now that the mesh is built from the same points, and (b) whether a ring can start filling immediately when a new puzzle appears if the child's hand is resting where they just answered and the new puzzle's option there has different text — this is mitigated by unlimited retries and is not a correctness bug, but is worth a comfortable-to-play check.
