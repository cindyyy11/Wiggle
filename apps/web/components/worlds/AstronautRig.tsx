...
// The whole visual rig; FloatingAstronaut only moves it, this component animates the limbs from the shared motion state.
export function AstronautRig({ motion, nozzles }: { motion: MutableRefObject<AstronautMotion>; nozzles: AstronautNozzles }) {
  ...
  return <>
    ...
    <group ref={helmet} position={[0, .56, 0]}>
      ...
      <mesh position={[0, .004, .112]} scale={[1, .8, .64]}><sphereGeometry args={[.182, 24, 16]} /><meshStandardMaterial color={NAVY} roughness={.5} /></mesh>
      ...
      <mesh position={[-.062, .062, .222]} scale={[1, .3, .1]} rotation={[0, 0, -.3]}><sphereGeometry args={[.053, 10, 6]} /><meshBasicMaterial color={GLINT} /></mesh>
      <mesh position={[.07, -.03, .222]} scale={[1, 1, .1]}><sphereGeometry args={[.016, 8, 6]} /><meshBasicMaterial color={GLINT} /></mesh>
