...
export function WorldSelector({ selectedWorld = "math", onSlide, onSelect, statusMessage }: WorldSelectorProps) {
  ...
  const atEnd = selectedWorld === SUBJECT_WORLD_ORDER[SUBJECT_WORLD_ORDER.length - 1];
  return <section className={`${styles.selector} ${styles.carouselSelector}`} aria-label="Choose a subject world">
    ...
    <button className={`${styles.carouselArrow} ${styles.arrowPrevious}`} aria-label="Previous planet" disabled={selectedWorld === SUBJECT_WORLD_ORDER[0]} onClick={() => onSlide?.(-1)}><ChevronLeft aria-hidden="true" /></button>
    <button className={`${styles.carouselArrow} ${styles.arrowNext}`} aria-label="Next planet" disabled={selectedWorld === SUBJECT_WORLD_ORDER[SUBJECT_WORLD_ORDER.length - 1]} onClick={() => onSlide?.(1)}><ChevronRight aria-hidden="true" /></button>
    {atEnd ? <p className={styles.moreWorldsHint} aria-hidden="true">✨ More worlds twinkling beyond</p> : null}
    <div className={styles.carouselCaption} data-mystery={world.mystery || undefined}>
      ...
      <button className={styles.carouselEnter} disabled={locked} aria-label={locked ? `${world.name} (coming soon)` : `Explore ${world.name}`} aria-describedby={locked ? "world-lock-status" : undefined} onClick={() => onSelect(selectedWorld)}>
        <span>{locked ? "Coming soon" : `Explore ${selectedWorld === "math" ? "Numeria" : "Science Planet"}`}</span>
        {locked ? <Lock aria-hidden="true" size={18} /> : <ArrowUpRight aria-hidden="true" size={18} />}
      </button>
    </div>
