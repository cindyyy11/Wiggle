...
export function WorldSelector({ selectedWorld = "math", onSlide, onSelect, statusMessage }: WorldSelectorProps) {
  ...
  return <section className={`${styles.selector} ${styles.carouselSelector}`} aria-label="Choose a subject world">
    ...
    <button className={`${styles.carouselArrow} ${styles.arrowPrevious}`} aria-label="Previous planet" disabled={selectedWorld === SUBJECT_WORLD_ORDER[0]} onClick={() => onSlide?.(-1)}><ChevronLeft /></button>
    <button className={`${styles.carouselArrow} ${styles.arrowNext}`} aria-label="Next planet" disabled={selectedWorld === SUBJECT_WORLD_ORDER[SUBJECT_WORLD_ORDER.length - 1]} onClick={() => onSlide?.(1)}><ChevronRight /></button>
