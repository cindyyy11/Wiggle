...
import type { UniverseCanvas } from "../universe/UniverseCanvas";
...
import { DEMO_CHILD_ID, DEMO_CORRECTNESS, DEMO_MISSION_ID, demoSession, demoSimulation, modeForStrategy } from "../../lib/demo/seed";
import type { FractionMissionProps, MissionPhase } from "./FractionMission";
...
/**
 * The world hosting the mission owns its camera and walking destination. The mission asks it to
 * frame the pizza table (`showMission`, also re-issued when the learning mode adapts) and to hand
 * the camera back when the child leaves (`hideMission`).
 */
export interface FractionMissionHost {
  showMission(): void;
  hideMission(): void;
  /** Called once per mission, when the child finishes it (not when they leave early). */
  missionCompleted?(): void;
}

export interface UseFractionMissionOptions {
  client?: ApiClient;
  childId?: string;
  allowLocalFallback?: boolean;
  onMissionOverlayChange?: (open: boolean) => void;
  host: FractionMissionHost;
}

export type FractionMissionPizza = NonNullable<ComponentProps<typeof UniverseCanvas>["pizza"]>;

export interface FractionMissionController {
  phase: MissionPhase | null;
  busy: boolean;
  feedback: string;
  /** Completed missions in this visit. */
  completed: number;
  missionOverlayOpen: boolean;
  start(): void;
  pizza: FractionMissionPizza;
  /** Props for FractionMission; null until a mission is open. */
  missionProps: FractionMissionProps | null;
}

export function useFractionMission({ client: suppliedClient, childId = DEMO_CHILD_ID, allowLocalFallback = true, onMissionOverlayChange, host }: UseFractionMissionOptions): FractionMissionController {
  ...
  // Report changes only: the overlay starts closed, so a mount-time "closed" is not news to the host.
  const reportedOverlay = useRef(false);
  useEffect(() => {
    if (reportedOverlay.current === missionOverlayOpen) return;
    reportedOverlay.current = missionOverlayOpen;
  ...
  const start = () => {
    ...
    void operation(async signal => {
      ...
      hostRef.current.showMission(); setPhase("standard");
      setNewStar(null);
  ...
  const finish = (resultMode: LearningMode) => {
    ...
    hostRef.current.missionCompleted?.();
  ...
  const close = () => {
    ...
    run.current = null; setPhase(null); setSlices([]); hostRef.current.hideMission(); setFeedback("");
  ...
  const pizza: FractionMissionPizza = { visible: pizzaVisible, selectedSlices: slices, slices: pizzaSlices, plate: { accepting: heldSlice !== null, focused: false }, hand: cameraEnabled ? { enabled: true, latest: handTracking.latest, gesture: handTracking.gesture, phase: gesturePhase, status: handTracking.status } : undefined, onGestureAction: handleGestureAction, onSliceSelect: commands.selectSlice };
  const missionProps: FractionMissionProps | null = phase ? {
    phase, mode, selectedSlices: slices, report, answer, feedback, busy, correctness, realityCompleted,
    onAnswer: value => { interact(); setAnswer(value); setFeedback(""); },
    onStuck: () => { interact(); setCameraEnabled(false); emit({ kind: "stuck_requested", mode }); setFeedback(""); setPhase("stuck"); supportReady.current = false; void operation(signal => adapt("visual_gesture", signal, true)); },
    onSimulate: simulate, onSelect: select, onCheck: check, onClose: close,
    onBack: () => setPhase(mode === "standard" ? "standard" : "activity"),
    commands, cameraEnabled, onCameraEnable: () => setCameraEnabled(true), onCameraDisable: () => { setCameraEnabled(false); setGesturePhase(null); setHeld(null); },
    tracking: handTracking, support, supportText, twinState: missionTwinState, newStar, greeting,
    onLexiRequest: requestLexi, onSupportClose: closeSupport, onSupportComplete: completeSupport,
  } : null;

  return { phase, busy, feedback, completed, missionOverlayOpen, start, pizza, missionProps };
}
