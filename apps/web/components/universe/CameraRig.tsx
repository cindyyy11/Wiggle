...
export function CameraRig({ mode, input, reducedMotion }: { mode: CameraMode; input: InputRef; reducedMotion: boolean }) {
  ...
  const scratch = useMemo(() => ({ target: new Vector3(), desired: new Vector3(), normal: new Vector3(), offset: new Vector3(), elevation: new Vector3(.2, 1.25, .8), previousAvatar: new Vector3(...input.current.position) }), [input]);
  ...
  useFrame((_, rawDelta) => {
    ...
    if (mode === "globe") { scratch.target.set(narrow ? 0 : -.32, narrow ? -1.15 : .05, 0); scratch.desired.set(narrow ? 0 : .6, narrow ? 1.5 : 1.45, narrow ? 23 : 10.8); }
    else {
    ...
    if (transition.current) {
      ...
      scratch.offset.copy(scratch.target).sub(scratch.previousAvatar);
      camera.position.add(scratch.offset); orbit.target.add(scratch.offset);
    }
    scratch.previousAvatar.set(...input.current.position);
  ...
  return <OrbitControls ref={controls} makeDefault enablePan={false} enableDamping={!reducedMotion} dampingFactor={.09} rotateSpeed={.65} zoomSpeed={.65} minDistance={mode === "globe" ? 7.2 : 1.7} maxDistance={mode === "globe" ? (narrow ? 30 : 17) : 5.3} minPolarAngle={.12} maxPolarAngle={Math.PI - .12} onStart={() => { transition.current = false; }} />;
}
