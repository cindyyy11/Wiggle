...
import type { UniverseSceneProps } from "./world";
...
export default function UniverseScene(props: UniverseSceneProps) {
  ...
  return <Canvas dpr={quality === "low" ? 1 : [1, 1.5]} camera={{ position: [.6, 1.45, 10.8], fov: 45, near: .1, far: 65 }} gl={{ antialias: quality === "high", alpha: true, powerPreference: "low-power", failIfMajorPerformanceCaveat: true }} fallback="Choose a destination using the map controls.">
    <ambientLight intensity={.85} color="#d8e7e4" />
    <hemisphereLight args={["#e0eee4", "#374355", 1.35]} />
    <directionalLight position={[-4, 8, 6]} intensity={2.6} color="#fff0d1" />
    <directionalLight position={[5, 2, -3]} intensity={1.3} color="#91cdd2" />
    ...
    <Landmarks selected={selectedLandmark} onSelect={onLandmarkSelect} reducedMotion={reducedMotion} mission={mode === "mission"} pizza={pizza} />
