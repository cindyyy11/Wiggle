...
export function Landmarks({ selected, onSelect, reducedMotion, mission, pizza, interactionTargets }: { selected: LandmarkId; onSelect: (id: LandmarkId) => void; reducedMotion: boolean; mission: boolean; pizza?: PizzaPresentation; interactionTargets?: SceneInteractionTargets }) {
  return <group>{LANDMARKS.map(item => <SurfaceAnchor key={item.id} destination={item.destination}>
    <group name={item.name} onClick={event => { if (event.delta > 7) return; event.stopPropagation(); onSelect(item.id); }}>
      <mesh position={[0, .045, 0]}><cylinderGeometry args={[item.id === "fraction-forest" ? .34 : .22, .27, .09, 8]} /><meshStandardMaterial color={item.id === "fraction-forest" ? "#fff7e7" : "#9dc99a"} roughness={1} flatShading /></mesh>
      {item.id === "fraction-forest" ? <MissionPedestal active={mission} pizza={pizza} interactionTargets={interactionTargets} /> : item.id === "lexi" ? <LexiBeacon reducedMotion={reducedMotion} interactionTargets={interactionTargets} /> : <group>
        <mesh position={[0, .3, 0]}><cylinderGeometry args={[.022, .034, .5, 5]} /><meshStandardMaterial color="#f4c95d" roughness={.9} /></mesh>
        ...
        <mesh position={[.125, .5, .029]}>{item.id === "geometry-ridge" ? <coneGeometry args={[.06, .1, 3]} /> : item.id === "crystal-crater" ? <octahedronGeometry args={[.06, 0]} /> : <boxGeometry args={[.035, .08, .015]} />}<meshStandardMaterial color="#285c85" roughness={.8} /></mesh>
      </group>}
...
function MissionPedestal({ active, pizza, interactionTargets }: { active: boolean; pizza?: PizzaPresentation; interactionTargets?: SceneInteractionTargets }) {
  return <group>
    <mesh position={[0, .15, 0]}><cylinderGeometry args={[.17, .21, .19, 8]} /><meshStandardMaterial color="#9dc99a" roughness={.9} /></mesh>
    <mesh position={[0, .28, 0]}><cylinderGeometry args={[.33, .24, .09, 8]} /><meshStandardMaterial color="#fff7e7" roughness={.82} /></mesh>
    <mesh position={[0, .335, 0]} rotation={[-Math.PI / 2, 0, 0]}><ringGeometry args={[.24, .27, 32]} /><meshBasicMaterial color="#f4c95d" /></mesh>
    {pizza?.visible ? <><PlateTarget presentation={pizza} interactionTargets={interactionTargets} /><PizzaModel presentation={pizza} interactionTargets={interactionTargets} /></> : <group position={[0, .57, 0]}>
      <mesh rotation={[0, Math.PI / 4, 0]}><octahedronGeometry args={[.16, 0]} /><meshStandardMaterial color="#fff7e7" emissive="#f4c95d" emissiveIntensity={active ? .48 : .2} roughness={.68} /></mesh>
      <mesh rotation={[Math.PI / 2, 0, 0]}><torusGeometry args={[.23, .013, 4, 24]} /><meshStandardMaterial color="#9dc99a" roughness={.9} /></mesh>
    </group>}
  </group>;
}
...
export function LexiBeacon({ reducedMotion, interactionTargets }: { reducedMotion: boolean; interactionTargets?: SceneInteractionTargets }) {
  ...
  return <group><mesh position={[0, .18, 0]}><cylinderGeometry args={[.07, .12, .26, 6]} /><meshStandardMaterial color="#9dc99a" roughness={.9} /></mesh><group ref={light} position={[0, .48, 0]}><mesh ref={mesh => registerInteractiveMesh(interactionTargets, mesh, LEXI_BEACON_ID, "lexi-beacon")}><octahedronGeometry args={[.16, 0]} /><meshStandardMaterial color="#fff7e7" emissive="#f4c95d" emissiveIntensity={.5} roughness={.65} /></mesh><mesh rotation={[0, 0, .6]}><torusGeometry args={[.235, .012, 4, 24]} /><meshBasicMaterial color="#f4c95d" /></mesh><mesh><sphereGeometry args={[.22, 10, 8]} /><meshBasicMaterial color="#a9d9ee" transparent opacity={.1} depthWrite={false} /></mesh></group></group>;
}
...
export function PizzaModel({ presentation, interactionTargets }: { presentation: PizzaPresentation; interactionTargets?: SceneInteractionTargets }) {
  return <group position={[0, .41, 0]} scale={1.2} name="Four equal pizza slices">{[0, 1, 2, 3].map(index => {
    ...
    return <group key={slice.id} position={position} name={slice.id} onClick={event => { if (event.delta > 7) return; event.stopPropagation(); presentation.onSliceSelect?.(index); }}>
      <mesh ref={mesh => registerInteractiveMesh(interactionTargets, mesh, slice.id, "pizza-slice")}><cylinderGeometry args={[.29, .29, .045, 10, 1, false, start, Math.PI / 2 - .05]} /><meshStandardMaterial color={placed ? "#f4c95d" : "#d9a755"} emissive="#f4c95d" emissiveIntensity={slice.focused || slice.state === "held" ? .18 : 0} roughness={.9} /></mesh>
      <mesh position={[0, .028, 0]}><cylinderGeometry args={[.256, .256, .019, 10, 1, false, start, Math.PI / 2 - .05]} /><meshStandardMaterial color={placed ? "#fff7e7" : "#f4c95d"} emissive="#f4c95d" emissiveIntensity={placed ? .18 : 0} roughness={1} /></mesh>
      {[.11, .2].map((radius, topping) => <mesh key={topping} position={[Math.sin(center + topping * .15) * radius, .044, Math.cos(center + topping * .15) * radius]} rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[.025, 7]} /><meshStandardMaterial color="#ef8b78" roughness={.9} /></mesh>)}
    </group>;
  })}</group>;
}
