...
const CHEER_BODY = "All four Science lands explored — you're a Science explorer!";
  ...
  return <ScienceSessionFrame open={!!session} name={scienceLand(session ?? selectedZone).name} onClose={close}>
    ...
    <UniverseCanvas explorerInput={input} controlsDisabled={paused} resetViewKey={resetKey} theme="science" className={styles.spaceWorld} quality={props.quality} reducedMotion={props.reducedMotion} mode={mode} destination={destination} onSceneAvailability={setAvailable}
      ...
      hud={<div className={styles.spaceHud}>
        <ScienceHud {...props} selectedZone={selectedZone} completedZones={completedZones} onExplore={explore} onStartMagnetLab={explore} onZoneSelect={select} completionMessage={returnMessage || undefined} />
    ...
    {active ? <div className={sessionStyles.magnetOverlay} data-session-controls><MagnetLabMission manageFocus={false} onExit={close} onComplete={() => completeZone("magnet-lab")} /></div> : null}
    {starter ? <ScienceHandSession key={starter} land={starter} progress={progress[starter]} onProgress={(value) => setProgress((previous) => ({ ...previous, [starter]: value }))} onComplete={completeZone} onClose={close} /> : null}
