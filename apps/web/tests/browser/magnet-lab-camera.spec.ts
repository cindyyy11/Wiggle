...
import { denyCamera, enterMagnetLab, enterScience, launchWiggle } from "./helpers";
...
test("Escape returns focus to the lab entry", async ({ page }) => {
  ...
  await expect(page.getByRole("button", { name: "Explore Magnet Lands", exact: true })).toBeFocused();
});
