...
export default function SignIn() {
  ...
  return <main className={styles.shell}><section className={styles.gate}><div className={styles.gateBadge} aria-hidden="true"><KeyRound /></div><p className={styles.eyebrow}>Welcome to your parent space</p><h1>Household sign-in</h1><p>Use the account connected to your explorer’s household.</p>
    <form onSubmit={async event => {
      ...
      <button disabled={busy} aria-busy={busy}>{busy ? "Signing in…" : "Sign in"}</button>{error && <p role="alert">{error}</p>}
    </form><a href="/" className={styles.backLink}><ArrowLeft className={styles.btnIcon} aria-hidden="true" />Back to the universe</a></section></main>;
}
